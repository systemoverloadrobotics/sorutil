...
/**
 * SensorConfiguration presents an abstracted interface for configuring sensor
 * values used on motors.
 * 
 * There are three classes of encoder sensors available:
 * <ul>
 * <li>IntegratedSensorSource: A sensor integral to the motor controller, only
 * supported on TalonFX (Falcon 500 motor) and SparkMax controllers.</li>
 * <li>ConnectedSensorSource: A sensor that is external to the controller, but
 * connected directly to it, for example using the SRX port on a TalonSRX.</li>
 * <li>ExternalSensorSource: A sensor that is not connected to the motor
 * controller, but instead connects to the RoboRio. Anything that extends the
 * WPILib Encoder class is usable in this mode.</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * If you use ConnectedSensorSource or ExternalSensorSource, you will need to
 * configure the encoder type separately.
 * </p>
 * 
 ...
 * </pre>
 * 
 * <h4>TalonSRX powered arm with 4:1 reduction before the sensor, and 23.4:1
 * reduction after the sensor, using external sensor</h4>
 * 
 * <pre>
 * var motorConfig = ...;
...
public class SensorConfiguration {
  ...
  /**
   * IntegratedSensorSource configures the motor controller to use the integrated
   * sensor available on a motor for the
   * sensing source.
   * 
   * Only the SparkMax (in brushless mode) and TalonFX support this. To use a
   * sensor connected directly to a TalonSRX, use
  ...
  /**
   * ExternalSensorSource configures the motor controller to use sensors connected
   * to the RoboRIO or provided from other sources.
   * 
   * <p>
   * This requires an instance of the WPILib Encoder class to be provided, which
   * is used as the source for the sensor. Note that using ExternalSensorSource
   * will force the library to resort to software based PID control, instead of
  ...
  /**
   * ConnectedSensorSource configures the motor controller to use sensors
   * connected directly to the motor controller (only supported by the TalonSRX
   * and SparkMAX).
   */
  ...
  /**
   * SensorSource describes what source the motor should use for closed loop
